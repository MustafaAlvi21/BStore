  ...
  <div class="container">
    <div class="row">
      ...
      <ul class="list-inline float-right top-right">
        <li class="account-login"><span><a href='{{ URL::to("login") }}'>Log in</a><small>or</small><a  href='{{ URL::to("register") }}'>Create an account</a></span></li>
      </ul>
      @else

      <ul class="list-inline float-right top-right">
        <li class="account-login"><span class="text-white">{{ Auth::user()->name }}<small> - </small>
      
        <a href='{{ URL::to("register") }}' onclick="event.preventDefault();
      ...
      @if(
        App\exchange::where('exchange_user', Auth::user()->id)->where('status', 'pending')->get()->count() > 0
      )
      <li class="dropdown cart-dropdown">
        <a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-bell"></i>{{ App\exchange::where('exchange_user', Auth::user()->id)->where('status', 'pending')->get()->count() }}</a>
        <ul class="dropdown-menu dropdown-menu-right">
          <li>Notificaitons product exchange.</li>
          @foreach(DB::table('exchanges')->join('users', 'users.id', '=', 'exchanges.user_id')->join('products', 'products.id', '=', 'exchanges.product_id')->where('exchanges.exchange_user', Auth::user()->id)->where('exchanges.status', 'pending')->select('users.name', 'products.name as p_name', 'products.id as p_id', 'exchanges.id as exc_id')->get() as $item)
          <li>
            <a href="{{ URL::to('exchange/'.$item->exc_id.'/view') }}" title="View Details">
              <div class="media">
                <!-- <img class="media-left media-object" src="img/home/cart-items/cart-item-01.jpg" alt="cart-Image"> -->
                <div class="media-body">
                  <h5 class="media-heading">Product exchange request from <br><span>{{ $item->name }}</span> product {{ $item->p_name }}</h5>
                </div>
              </div>
            </a>
            <div class="btn-group" role="group" aria-label="...">
              <form action="{{ URL::to('accept_request/'.$item->exc_id) }}" method="post">
              @csrf
              <button type="submit" class="btn btn-default" title="Accept">Accept</button>
              </form>
              <form action="{{ URL::to('ignore_request/'.$item->exc_id) }}" method="post">
              @csrf
              <button type="submit" class="btn btn-default" title="Igonre">Ignore</button>
              </form>
            </div>
...
<div class="navTop text-center">
  <div class="container">
      ...
      <img src="{{ asset('web_assets/img/logo.png') }}" alt="">

      </a>

      <div class="navTop-middle" id="hide-search-btn-in-mobile">
      ...
      <div class="searchBox">
        <span class="input-group">
          <input id="searchbox" type="text" name="search" class="form-control" placeholder="Search for skills, products and friends." aria-describedby="basic-addon2">
    ...
    <div class="cart-dropdown">
      <a href="{{ URL::to('my_cart') }}" class="shop-cart">
        <i class="fa fa-shopping-cart"> <small style="background-color: orange!important;padding: .2em;border-radius: 30px;">{{ Cart::getContent()->count() }}</small> </i>
        <!-- <span class="d-none d-md-block">
          <span class="cart-total">Your Cart (0)</span><br>
          <span class="cart-price">Rs 00.00</span>
        </span> -->
      </a>
      <!-- <ul class="dropdown-menu dropdown-menu-right">
        ...
        <li>
          <a href="#">
            <div class="media">
              <img class="media-left media-object" src="{{ asset('web_assets/img/home/cart-items/cart-item-01.jpg') }}" alt="cart-Image">
              <div class="media-body">
